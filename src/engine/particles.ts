...
const create_particle_emitter: (x: number, y: number, system: ParticleSystem, emission_enabled?: boolean, enabled?: boolean) => ParticleEmitter = (x, y, system, emission_enabled = true, enabled = true) => ({
    x,
    y,
    id: Math.random().toString(36).replace(/[^a-z]+/g, '').substr(2, 10),
    particles: [],
